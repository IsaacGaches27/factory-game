...
fn camera_controller(keys: Res<Input<KeyCode>>,mut camera_query: Query<(&mut Transform,&mut OrthographicProjection)>){
    ...
    if keys.pressed(KeyCode::W) {
        transform.translation = Vec3::new(transform.translation.x,transform.translation.y + 15.,0.);
    }
    if keys.pressed(KeyCode::A) {
        transform.translation = Vec3::new(transform.translation.x - 15.,transform.translation.y,0.);
    }
